...
<article class="flex space-x-4 rtl:space-x-reverse">
	{#if comment.expand?.user}
		<div class="flex-shrink-0">
			<div class="flex h-12 w-12 items-center justify-center rounded-full bg-gray-700 font-bold text-white">
		...
		<div class="flex-grow">
			...
			<div class="mt-2 flex items-center gap-4 text-xs text-gray-400">
				<form method="POST" action="?/toggleLike" use:enhance>
					...
					<button type="submit" class="hover:text-white flex items-center gap-1">
			...
			{#if showReplyForm}
				<form method="POST" action="?/addComment" use:enhance class="mt-4 ml-4 rtl:mr-4">
					...
					<button
						type="submit"
						class="mt-2 rounded bg-orange-600 py-1 px-4 text-sm font-bold text-white hover:bg-orange-700"
					>
						إرسال الرد
					</button>
				</form>
			...
			{#if comment.replies && comment.replies.length > 0}
				<div class="mt-4 space-y-4 border-r-2 border-gray-700 pr-4 rtl:border-r-0 rtl:border-l-2 rtl:pr-0 rtl:pl-4">
