...
<a
	...
	class="group flex transform flex-col overflow-hidden rounded-lg bg-gray-800 shadow-lg transition-transform duration-300 hover:-translate-y-2"
>
	<div class="relative h-48 w-full">
		...
		<div class="absolute top-2 right-2 flex gap-2">
			<span
				class="rounded-full bg-blue-500/80 px-3 py-1 text-xs font-bold text-white backdrop-blur-sm"
			>
				{quiz.difficulty}
			</span>
			<span
				class="rounded-full bg-purple-500/80 px-3 py-1 text-xs font-bold text-white backdrop-blur-sm"
			>
				{quiz.category}
			</span>
		</div>
	...
	<div class="flex flex-grow flex-col p-6 text-right">
		...
		<p class="mt-2 line-clamp-2 flex-grow text-gray-400">{quiz.description}</p>
