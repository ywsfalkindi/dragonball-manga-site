...
<div class="min-h-screen bg-gray-900 p-8 font-[Tajawal] text-white">
	...
	<h1 class="mb-8 text-right text-4xl font-bold">إعدادات الموقع العامة</h1>

	<div class="mx-auto max-w-2xl space-y-8">
		<div class="rounded-lg bg-gray-800 p-6 shadow-lg">
			<h2 class="mb-4 text-2xl font-bold">وضع الصيانة</h2>
			<form method="POST" action="?/toggleMaintenance" class="flex items-center justify-between">
				<p class="text-gray-400">عند التفعيل، لن يتمكن أحد من تصفح الموقع باستثناء المسؤولين.</p>
				<button
					type="submit"
					class="rounded px-6 py-2 font-bold transition-colors {maintenanceMode
		...
		<div class="rounded-lg bg-gray-800 p-6 shadow-lg">
			<h2 class="mb-4 text-2xl font-bold">إعلان الموقع</h2>
			<form method="POST" action="?/updateAnnouncement" class="space-y-4">
				<div>
					<label for="announcement" class="mb-2 block text-gray-300">نص الإعلان</label>
					<textarea
						name="text"
						id="announcement"
				...
				<div class="flex items-center gap-4">
					...
					<input
						...
						name="isActive"
						id="isActive"
						bind:checked={announcementActive}
