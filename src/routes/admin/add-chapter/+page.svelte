...
<div class="p-8 font-[Tajawal] bg-gray-900 min-h-screen text-white">
	...
	<h1 class="text-4xl font-bold mb-8">أداة إضافة فصل جديد</h1>

	<form method="POST" class="max-w-xl bg-gray-800 p-8 rounded-lg shadow-lg space-y-6">
		<div>
			<label for="manga" class="block text-gray-300 mb-2">اختر المانجا</label>
			<select name="mangaId" id="manga" class="w-full bg-gray-700 text-white rounded p-2 border border-gray-600" required>
		...
		<div>
			...
			<input type="number" name="chapterNumber" id="chapterNumber" class="w-full bg-gray-700 text-white rounded p-2 border border-gray-600" required />
		</div>
		<div>
			<label for="totalPages" class="block text-gray-300 mb-2">إجمالي عدد الصفحات</label>
			<input type="number" name="totalPages" id="totalPages" class="w-full bg-gray-700 text-white rounded p-2 border border-gray-600" required />
		</div>
		...
		<button type="submit" class="w-full bg-green-600 text-white font-bold py-3 px-4 rounded hover:bg-green-700 transition-colors">
			بدء الإضافة
		</button>
	</form>
</div>